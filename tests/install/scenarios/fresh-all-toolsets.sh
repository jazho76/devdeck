#!/usr/bin/env bash
set -euo pipefail

source /opt/devdeck-src/tests/install/lib/assert.sh
source /opt/devdeck-src/tests/install/lib/env.sh
source /opt/devdeck-src/tests/install/lib/state.sh

setup_test_home
install_devdeck_cli_from_source

note "Installing with all Neovim toolsets"
devdeck install --all-toolsets --skip-lazy-install

assert_devdeck_cli
assert_devdeck_source
assert_tmux_config_link
assert_tpm_installed
assert_nvim_config_link
assert_nvim_available
assert_nvim_binary_starts

toolsets_file="$HOME/.local/share/devdeck/source/nvim/lua/config/toolsets-local.lua"
for toolset in "$HOME/.local/share/devdeck/source/nvim/lua/toolsets"/*.lua; do
  name="$(basename "$toolset" .lua)"
  assert_contains "$toolsets_file" "'$name'"
done
